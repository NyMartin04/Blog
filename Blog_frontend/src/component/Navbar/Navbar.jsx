    ...
    return (
        <div className='shadow-2xl'>
            <div className='flex justify-end items-center py-8 w-full bg-gray h-[5vh] '>
                <form className='mx-2' onSubmit={(e)=>searchUser(e)}>
                    <input className='min-w-[100px] max-w-[130px] bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500' onChange={(e)=>{
                    if (e.target.value.length >= 2) {
                        setSearchData(e.target.value); searchUser(e)
                    }else{
                        setSearchValue(false);
                    }
                    ...
                    {searchValue.map((item,index)=>{
                        return (<div className='flex justify-center items-center '><div className='flex justify-between items-center hover:bg-white hover:text-black text-white bg-black min-w-[200px] px-3 py-1 max-w-[400px]'>
                             {item.profilePicture === null? <img src={account} />:<img src={item.profilePicture}/>}
